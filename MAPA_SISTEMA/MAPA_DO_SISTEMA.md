...
- **Relatórios:** Um único item **Relatórios** → `/negocio/relatorios` (visível para quem tem `certificacao:relatorios:visualizar` ou `negocios.venda:visualizar`). A rota `/relatorios` redireciona (302) para `/negocio/relatorios` (unificação).
- **Configurações** (seção): Configurações (`/configuracoes`), **Empresa Fiscal** (`/fiscal/empresa` — SuperAdmin define **Modo de Recebimento** por empresa: direto = CA recebe no gateway; plataforma = plataforma recebe e repassa; e taxas da plataforma), Config. NFS-e (`/fiscal/nfse-config`), Pendências NFS-e (`/fiscal/nfse-pendencias`), Regras Fiscais ICMS (`/fiscal/regras-fiscais-icms`). Visível por permissão `fiscal.empresa` ou `fiscal.notas-fiscais` (itens fiscais); Superadmin/Admin para Configurações.
...
- **Superadmin — SEO avançado (telas internas):** `GET /admin/marketplace-seo-lojas` — edição de `seo_title`, `seo_description`, `og_image_url`, `seo_enabled`, textos e nome fantasia por loja; apenas Superadministrador; consome APIs em `app/api/v1/marketplace.py`. Sidebar: item **SEO vitrine (lojas)**.
...
- **Detalhe do produto (UX vitrine):** Em `GET /loja/produto/{id}`, abaixo da galeria são exibidos (quando houver dados): card **Descrição do CA** (`produtos_cliente.descricao`) e card **Produtos semelhantes** (mesma categoria de estoque, aleatórios, excluindo o anúncio atual) via endpoint público `GET /api/v1/loja/anuncios/{anuncio_id}/semelhantes`.
...
- `app/templates/base.html` - Navbar com botão WhatsApp (SVG), dropdown de alertas, mensagens, user dropdown; painel chat (slide-over) e overlay. Todas as páginas que estendem `base.html` compartilham esse cabeçalho.
...
**Gestão (APIs e HTML):** Rotas em `app/api/v1/marketplace.py`. CRUD categorias plataforma; GET/POST/PATCH loja (por cliente_id no escopo); **GET `/api/v1/marketplace/lojas`** (listagem global, apenas Superadministrador); CRUD anúncios (escopo por loja/cliente); POST sync/estoque (atualiza estoque_atual dos anúncios a partir de produtos_cliente; registra em SyncControle); GET loja/{id}/pedidos, PATCH pedidos/{id} (status); GET loja/{id}/extrato. **PATCH loja:** campos de frete da loja e campos de SEO avançado (`seo_title`, `seo_description`, `og_image_url`, `seo_enabled`) restritos a Superadministrador (ver MAPA_DE_API § 19). Rotas HTML: `/negocio/marketplace`, `/negocio/marketplace/minha-loja` (exigem auth e `marketplace:visualizar`); **`/admin/marketplace-seo-lojas`** (apenas Superadministrador, SEO avançado por loja). Escopo: ClienteScope (Superadmin vê tudo; Admin/CA filtram por allowed_ids).

**Marketing Vitrine (home — config global):** **Regra de produto (gravada):** todos os **cards** de marketing da home (faixa destaques, ofertas da semana, cabeçalho de ofertas) são **configurados e parametrizados exclusivamente** pelo **Superadministrador** em **`/admin/marketing-vitrine`**; não há outra tela no sistema para esses registros. Tabelas `marketing_vitrine_config` (singleton `id=1`) e `marketing_vitrine_cards` (blocos `destaque` e `oferta_semana`; cards `livre`, `anuncio` ou `cabecalho_ofertas`; migrações **mv01** + **mv02** + **mv03** (`mv03_mv_sec_defaults`) — flags e títulos das seções da home: hero, faixa destaques, em alta, lojas em destaque; defaults de “em alta” e “lojas” em **true** para manter o mesmo desenho da home anterior). API `app/api/v1/marketing_vitrine.py`: **GET** `/api/v1/marketing-vitrine/vitrine-home` — público, payload `config` + `destaques` + `ofertas_semana` + `generated_at`, cabeçalhos `Cache-Control: no-store`; **GET/PATCH** `/api/v1/marketing-vitrine/config` e **GET/POST/PATCH/DELETE** `/api/v1/marketing-vitrine/cards` — apenas Superadministrador (`require_superadmin`). Serviço: `app/services/marketing_vitrine_service.py`. HTML **`/admin/marketing-vitrine`** (template `app/templates/admin/marketing_vitrine.html`); item de menu no sidebar só Superadmin. A home pública **`/loja`** (`app/templates/loja/index.html`) recebe `marketing_vitrine` em `_loja_context` (servidor) para exibir ou ocultar blocos estáticos (hero, em alta, lojas) e títulos; consome a API via `app/static/js/vitrine.js` (`getMarketingVitrineHome`, `cache: no-store`) para montar destaques e ofertas; pode ocultar a seção «Todos os produtos» conforme `mostrar_todos_produtos`. Ver MAPA_DE_API § 19.
...
**Ecossistema pós-checkout (NF-e automática e notificação CA):** Após checkout (POST /api/v1/loja/checkout) são enfileiradas as tasks Celery `emitir_nfe_pedido_marketplace` (cria NotaFiscal a partir de PedidoMarketplace e envia à SEFAZ; destinatário = dados do pedido, sem criar cliente) e `notificar_ca_novo_pedido` (e-mail aos responsáveis da loja). Notas de venda marketplace: `notas_fiscais.pedido_marketplace_id`, origem VENDA_MARKETPLACE. **Minha loja (100% pelo front):** Tela `/negocio/marketplace/minha-loja` — ativar loja (botão), editar dados da loja (form PATCH), listar/publicar/editar/pausar anúncios, sincronizar estoque, listar pedidos (PATCH status), extrato. Integração sem hardcode; contexto `minha_loja_cliente_id` na rota. **Pedidos Marketplace em /negocio/pedidos:** A tela `/negocio/pedidos` possui aba "Pedidos Marketplace" (primeira aba, visível para quem tem `marketplace:visualizar` ou `marketplace:gerenciar_pedidos`), listando pedidos do marketplace do estabelecimento com filtro e alteração de status; a lista de status é global e configurável pelo Superadministrador na mesma tela (tabela `status_pedido_marketplace`; endpoints GET/POST/PATCH e PATCH .../desativar em `/api/v1/marketplace/status-pedido`). Validação: PATCH em pedidos da loja exige que `status_pedido` seja um código ativo em `status_pedido_marketplace`.
...
- **Dashboard:** `http://127.0.0.1:8000/dashboard` — Hub por perfil: usuários com permissão `negocios` ou `pdv` veem o Dashboard PDV; demais veem o dashboard de certificação (agendamentos, equipamentos).
- **Dashboard PDV (Negócios):** `http://127.0.0.1:8000/negocio/dashboard` — KPIs PDV: faturamento, vendas, ticket médio, produtos por venda, cancelamentos, estoque, ordens de serviço.
...
- **Tela e API:** Rota `GET /fiscal/regras-fiscais-icms`; template `app/templates/fiscal/regras_fiscais_icms.html`; API CRUD `/api/v1/fiscal/regras-fiscais-icms`. Permissões: `fiscal.empresa` ou `fiscal.notas-fiscais`. Sidebar: **Regras Fiscais ICMS** na seção Configurações.
...
- **Rota e tela:** `GET /fiscal/nfse-config` renderiza `app/templates/fiscal/nfse_config.html`. Acesso com permissão `fiscal.empresa`; rota em `main.py` com checagem `_fiscal_nfse_has_permission`.
- **Sidebar:** O item "Config. NFS-e" foi removido do menu (2026-03-03). A seção **Configurações** do sidebar contém apenas: Configurações, Empresa Fiscal, Pendências NFS-e. A rota e a tela `/fiscal/nfse-config` continuam disponíveis para quem tem `fiscal.empresa` (acesso direto por URL). Fonte: `app/templates/components/sidebar.html`.
