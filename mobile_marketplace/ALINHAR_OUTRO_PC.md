...
> **VPS:** backend e app ficam na mesma máquina sob **`/central_solumatica/pdv_solumatica/`**. O Git pode estar na **raiz** desse caminho (monorepo) ou, no futuro, só em subpastas — o que importa para **publicar o mobile** é o script na raiz: **`./scripts/sync-mobile-to-github.sh`** → GitHub [`IBIX_mobile`](https://github.com/jkhons/IBIX_mobile). O PDV **sem** `mobile_marketplace/` → [`ibix_market`](https://github.com/jkhons/ibix_market) via **`./scripts/push_root_sem_mobile_ibix_market.sh`**. Ver **[`../REPOSITORIOS_GITHUB.md`](../REPOSITORIOS_GITHUB.md)** (fonte de verdade = VPS; cuidado com `git pull`).
